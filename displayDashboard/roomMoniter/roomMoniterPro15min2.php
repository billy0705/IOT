		...
		<script>
			...
			/* function handleInputData(timearray, temperaturearray, humidityarray) {
				...
				for (let i = 0; i < timearray.length; i++){
					...
					date.setUTCHours(time.getUTCHours() + 7);
					//console.log(time);
				...
				for (var i = 0; i < timeArray.length; i++){
					...
					if (i == 0){
						if (time.getUTCHours() != 0){
							timeoffset = 24 - time.getUTCHours();
						}
					}
					...
					//
					//console.log(time);
					var roundedTime = new Date(Math.ceil(time.getTime() / (15 * 60 * 1000)) * 15 * 60 * 1000);
